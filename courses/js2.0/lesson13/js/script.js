...
// document.querySelector('.out').innerHTML = a i can't do it
...
function f7() {
    ...
    for (key in a7) {
        if (Object.keys(a7).includes(input)) {
...
function f11() {
    ...
    for (key in a11) {
        if (Object.keys(a11).includes(input)) {
            delete a11[input]
        }
    }
...
function f18() {
    ...
    for (key in a18) {
        if (Object.keys(a18).includes(input)) {
    ...
    document.querySelector('.out-18').innerHTML = station
...
function f19() {
    ...
    for (key in a19) {
        for (let i = 0; i < a19[key].length; i++) {
            if (a19[key][i].toLowerCase() == input || a19[key][i] == input) {
                color = key
            }
        }
    }

    document.querySelector('.out-19').innerHTML = color
